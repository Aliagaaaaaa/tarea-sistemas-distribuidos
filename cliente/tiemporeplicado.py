...
def calculate_time(summoner_id, client):
    start_time = time.time()
    get_stats(summoner_id, client)
    end_time = time.time()
...
def process_summoner_ids(summoner_ids, client):
    ...
    for summoner_id in summoner_ids:
        time = calculate_time(summoner_id, client)
        total += time
        i += 1
        if i % 1000 == 0:
            print(f'Thread {client} - {i} summonerids procesados. Tiempo total: {total:.2f} segundos')
